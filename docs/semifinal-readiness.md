...
| 工程验证与安全审计      |  20% | HMAC、幂等、显式状态机、一次性审批、Evidence 哈希链     | 新增 Proof Bundle、PR Proof Comment、确定性评分、Agent/Skill/MCP/Run Span 和 Metrics、26/26 可靠性报告 | 部署 OTLP 后端并保留真实 Trace/Metrics 截图或导出文件               |
...
CI 运行 `pnpm benchmark:reliability`，当前本地基线为 **26/26 tests passed**。该报告只
验证确定性工程契约，包括：
